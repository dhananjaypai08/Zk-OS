...
void AddressVerifier_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void AddressVerifier_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Bits2Num_0_run,
IsZero_1_run,
IsEqual_2_run,
AddressVerifier_3_run };
...
uint get_main_input_signal_no() {return 161;}

uint get_total_signal_no() {return 329;}

uint get_number_of_components() {return 4;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 165;}
...
uint aux_create = 0;
int aux_cmp_num = 0+ctx_index+1;
uint csoffset = mySignalStart+161;
for (uint i = 0; i < 1; i++) {
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[0]); // line circom 14
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[2]); // line circom 24
}}
...
AddressVerifier_3_create(1,0,ctx,"main",0);
AddressVerifier_3_run(0,ctx);
}
